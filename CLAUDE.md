...
**Описание:** Сводная страница по всем объектам компании (иерархия смарт-процессов
Сделка → Этап (Milestone) → Модуль в Б24, портал «Альфа») для директора и его
замов — сроки и ключевые показатели по каждому объекту без погружения в детали
конкретной сделки. Не вкладка в карточке — отдельная страница в LEFT_MENU.
